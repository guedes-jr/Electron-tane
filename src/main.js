...
ipcMain.handle('clients:delete-many', (_, clientIds = []) => {
 ...
 ipcMain.handle('invoices:save', (_, payload) => {
   ...
   const invoice = {
     ...
     discountPercent,
     taneTotal,
     enelTotal: Number(payload.enelTotal || 0),
     savedAmount,
     totalKwh: Number(payload.totalKwh || 0),
     billedKwh: Number(payload.billedKwh || 0),
     compensatedKwh: Number(payload.compensatedKwh || 0),
     kwhRate: Number(payload.kwhRate || 0.93),
     accumulatedKwh: payload.accumulatedKwh != null && payload.accumulatedKwh !== '' ? Number(payload.accumulatedKwh) : null,
